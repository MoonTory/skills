---
name: explore
description: Explore and trace an unfamiliar subsystem before planning or changing it. Use for "how does this work", code walkthroughs, root-cause research, ownership or layering questions, and the Explore phase of a teamlead workflow. Can add a separate architecture critique when asked.
---
...
Use one read-only explorer that both traces and explains. The lead may do this directly when delegation would add
more work than the question.
...
Once all explorers return, use a strong judgment model or the lead to synthesize one coherent explanation. Give the
synthesizer all findings and their source paths. It stays read-only.
...
Use the `adversarial-review` judgment framework. The lead decides; it does not count votes.
