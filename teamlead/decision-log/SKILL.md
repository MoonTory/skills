---
name: decision-log
description: Keep a small, reviewable TSV trail for long-running or unattended work, with one row per real decision or verified checkpoint. Use for multi-phase runs, orchestrate, custom workflows, or work the user will review after stepping away.
---
...
Write each entry the way you would tell a teammate. Use plain words and concrete actions. A reviewer should understand
each row without decoding it.
...
- Prefer evidence another engineer can rerun over a one-off claim.

## Audit the log against the transcript

At the end of the run, check that the log tells the truth. Use the active conversation or its known transcript. Read
local transcripts only within the workspace, harnesses, and time range the user approved. Walk the log against what
actually happened:
...
For a large or high-risk run, ask a read-only agent from a different model family to scan the trail and relevant
transcript. Skip this for a small trail where the extra review would cost more than the work. The scan flags what the
user should pay attention to; it does not redo the task.
...
When a trail review ran, include its model and flags in the handoff. `No flags` is valid. Do not add an empty section
when the review was intentionally skipped.
