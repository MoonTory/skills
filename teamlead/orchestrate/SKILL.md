...
Create `orchestrate/<project-slug>/` in the current lead's durable workspace. Every file has exactly one writer;
owners publish facts and readers aggregate. Set `ORCH_STORE` to that directory or pass `--store <dir>` when running
...
A track-teamlead brief adds its tab or track boundary, unit list, spawn budget, drain protocol, and compact rollup
format.
...
1. **Frame.** State a countable done predicate. Quantify units, effort, tracks, and time. If one agent can finish inside
   ...
   `arena` for a contested decomposition. Review the program contract with the user once before scaling.
2. **Install the runtime.** Run `orch init`. Open `decision-log`, write standing orders before any spawn, and seed
...
7. **Close.** Drain the inbox, reconcile every spawned agent to a terminal row, confirm the predicate on the real
   artifact, confirm every landed PR has a current verdict, audit the decision trail, and encode recurring corrections
   into standing orders, a skill, or a check. Leave the store intact for review.
...
- The frontier is computed, never narrative. When the project uses Graphite, recompute `frontier.json` with `gt`
  after every merge or stack change. Without Graphite, track the ordered PRs and exact head SHAs in the repository's
  normal source of truth and skip the `orch frontier` commands.
- Exactly one stacker per stack may restack or retarget it. Record the owner in standing orders.
- Workers never rebase or mutate a shared stack unless their brief makes them its stacker.
