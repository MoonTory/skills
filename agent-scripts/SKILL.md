...
- `agent-task` waits for startup, prompts once, and waits for settle; `--timeout 0` returns after the prompt with the current status; exits 0 settled, 2 gone, 3 timeout, or 4 prompt/tool failure.
